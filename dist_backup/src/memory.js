...
async function summarizeMessages(messages) {
    ...
    const conversationText = messages.map(m => {
        ...
        if (typeof m.content === "string") {
            ...
            text = m.content.map((c) => c.type === 'image_url' ? '[IMAGEM CAPTURADA PELO NAVEGADOR]' : c.text).join(" ");
        }
        else {
    ...
    try {
        const response = await llmClient_1.openai.chat.completions.create({
